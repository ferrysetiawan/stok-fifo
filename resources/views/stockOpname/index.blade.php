@extends('layouts.global')

@section('title')
    Stock Opname
@endsection

@section('content')
<section class="section">
    <div class="section-header d-flex justify-content-between">
        <h1>Halaman Stock Opname</h1>
        <div class="d-flex">
            <select id="bulan" class="form-control mr-2">
                <option value="">Pilih Bulan</option>
                @for ($i = 1; $i <= 12; $i++)
                    <option value="{{ $i }}" {{ now()->month == $i ? 'selected' : '' }}>{{ DateTime::createFromFormat('!m', $i)->format('F') }}</option>
                @endfor
            </select>
            <select id="tahun" class="form-control mr-2">
                <option value="">Pilih Tahun</option>
                @for ($i = now()->year; $i >= 2000; $i--)
                    <option value="{{ $i }}" {{ now()->year == $i ? 'selected' : '' }}>{{ $i }}</option>
                @endfor
            </select>
            <button id="filterButton" class="btn btn-success px-4">Filter</button>
            <a href="{{ route('stockOpname.format') }}" target="_blank" class="ml-2 btn btn-light">
                Format Stock Opname
            </a>
        </div>
    </div>
    <div class="section-body">
        <div class="row my-5">
            <div class="col-lg-12">
                <div class="card shadow">
                    <div class="card-header bg-primary d-flex justify-content-between align-items-center">
                        <h3 class="text-light">Daftar Stock Opname</h3>
                        <a href="{{ route('stockOpname.create') }}" class="btn btn-light">
                            Tambah Stock Opname
                        </a>
                    </div>
                    <div class="">
                        <div class="card-body">
                            <table id="table-stokopname" class="table table-bordered table-md">
                                <thead>
                                    <tr>
                                        <th>No</th>
                                        <th>Nama Barang</th>
                                        <th>Kategori</th>
                                        <th>Satuan</th>
                                        <th>Jumlah Awal</th>
                                        <th>Penerimaan</th>
                                        <th>Pengeluaran</th>
                                        <th>Jumlah Akhir</th>
                                        <th>Stok Fisik</th>
                                        <th>Selisih</th>
                                        <th>Aksi</th>
                                    </tr>
                                </thead>
                            </table>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
@endsection

@section('js')
<script>
     $(document).ready(function () {
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
            });

            var table = $('#table-stokopname').DataTable({
                // processing: true,
                serverSide: true,
                responsive: true,
                searching: true,
                paging: true,
                info: false,
                ordering: false,
                ajax: {
                    url: "{{ route('stockOpname.index') }}",
                    data: function (d) {
                        d.bulan = $('#bulan').val();
                        d.tahun = $('#tahun').val();
                    }
                },
                columns: [
                    {
                        data: null,
                        render: function (data, type, row, meta) {
                            return meta.row + meta.settings._iDisplayStart + 1; // Menampilkan nomor urut
                        }
                    },
                    { data: 'bahan_baku', name: 'bahan_baku_id' },
                    { data: 'kategori', name: 'bahanBaku.kategori_id' },
                    { data: 'satuan', name: 'bahanBaku.satuan' },
                    { data: 'stok_awal', name: 'stok_awal' },
                    { data: 'penerimaan', name: 'penerimaan' },
                    { data: 'pengeluaran', name: 'pengeluaran' },
                    { data: 'stok_akhir', name: 'stok_akhir' },
                    { data: 'stok_fisik', name: 'stok_fisik' },
                    { data: 'selisih', name: 'selisih' },
                    {
                        data: null,
                        render: function (data) {
                            return '<a class="btn btn-warning mr-1" data-toggle="tooltip" data-placement="bottom" title="ubah" href="{{ url('stock-opname') }}/' + data.id + '/edit"><i class="fas fa-pencil-alt"></i></a>';
                        }
                    }
                ]
            });

            $('#filterButton').click(function() {
                table.ajax.reload();
            });
        });
</script>
@endsection
